...
void GREEDY:: Greedy_Rabin_Karp_Driver()
{
    ...
    {
	cout << "   rabin carp" << " " << Pattern_Code.length() << "  " << Text_Code.length() << endl;
...
void GREEDY :: tiling()
{
    ...
    {
        cout << "Tiles = " << " " << "Index_Pattern  = " << Tiles[i].getIndex_Pattern() << " Index_Text = " << Tiles[i].getIndex_Text() << " length = " << Tiles[i].getToken_Matched_Length() << endl ;
    }
}
...
void GREEDY :: Greedy_string_tiling (){
  ...
  unsigned  int Text_Length=Text_Code.length();
    do{
        ...
        for(unsigned int i=0;i<Pattern_Length /*&& Visited_Pattern[i]==0*/;i++){//a b g h h h g
            for(unsigned int j=0;j<Text_Length /*&& Visited_Text[j]==0*/;j++){// a f b g h g
